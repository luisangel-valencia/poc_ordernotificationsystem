...
/// <summary>
/// Lambda function handler for processing order email notifications from SQS
/// </summary>
public class Function
{
    ...
    public Function() : this(
        _serviceProvider.Value.GetRequiredService<IEmailService>(),
        _serviceProvider.Value.GetRequiredService<ILogger<Function>>())
    {
    }

    // Constructor for dependency injection (testing)
    public Function(IEmailService emailService, ILogger<Function> logger)
    {
    ...
    /// <summary>
    /// Lambda function handler for processing SQS events containing order notifications
    /// </summary>
    ...
    private OrderEvent? ParseOrderEvent(string messageBody)
    {
        try
        {
            ...
            if (string.IsNullOrEmpty(orderEvent.OrderId) || 
                string.IsNullOrEmpty(orderEvent.CustomerEmail))
            {
                _logger.LogError("Order event missing required fields: OrderId or CustomerEmail");
                return null;
            }
